...
public class FeignOutboundMetricsMethodHandlerTest {
    ...
    public void nothingIsMetered() {
        final MyClientWithoutAnnotation target = Feign.builder().invocationHandlerFactory(
    ...
    public void timedPerClassMethodsAreTimed() {
        final MyClientWithAnnotationOnClassLevel target = Feign.builder().invocationHandlerFactory(
                new FeignMetricsInvocationHandlerFactoryDecorator(new InvocationHandlerFactory.Default(),
                        metricRegistry)).target(MyClientWithAnnotationOnClassLevel.class,
    ...
    public void timedPerMethodsAreTimed() {
        final MyClientWithAnnotationOnMethodLevel target = Feign.builder().invocationHandlerFactory(
                new FeignMetricsInvocationHandlerFactoryDecorator(new InvocationHandlerFactory.Default(),
                        metricRegistry)).target(MyClientWithAnnotationOnMethodLevel.class,
                String.format("http://localhost:%d", wireMockRule.port()));
